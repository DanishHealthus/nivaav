import { FooterLinkSection } from "./allTypes";
import locationIcon from "../../../public/locationIcon.webp";

export const Footer3Links: FooterLinkSection[] = [
    {
      id: 1,
      titleIcon: locationIcon,
      title: "For Enquiries",
      links: [
        { subTitle: "care@nivaancare.com", url: "mailto:care@nivaancare.com" },
        { subTitle: "+91-63 6652 5217", url: "tel:+916366525217" },
      ],
    },
    {
      id: 2,
      title: "Conditions We Treat",
      links: [
        {
          subTitle: "Lower Back Pain",
          url: "https://www.nivaancare.com/back-pain-treatment/",
        },
        {
          subTitle: "Knee Pain",
          url: "https://www.nivaancare.com/knee-pain-treatment/",
        },
      ],
    },
    {
      id: 3,
      title: "Quick Links",
      links: [
        {
          subTitle: "Pain Management - Delhi",
          url: "https://www.nivaancare.com/pain-management-clinic-in-delhi/",
        },
      ],
    },
    {
      id: 4,
      titleIcon: locationIcon,
      title: "Our Locations",
      links: [
        {
          subTitle: "South-Delhi",
          url: "https://maps.app.goo.gl/ShnPReST8DKN9gRs5",
        },
        {
          subTitle: "North-Delhi",
          url: "https://maps.app.goo.gl/3Dg5FdmaM87GsS2UA",
        },
      ],
    },
  ];
  
  // Footer2 Links
  export const Footer2Links: FooterLinkSection[] = [
    {
      id: 1,
      titleIcon: locationIcon,
      title: "For Enquiries",
      links: [
        { subTitle: "care@nivaancare.com", url: "mailto:care@nivaancare.com" },
        { subTitle: "+91-63 6652 5217", url: "tel:+916366525217" },
      ],
    },
    {
      id: 2,
      title: "Conditions We Treat",
      links: [
        {
          subTitle: "Lower Back Pain",
          url: "https://www.nivaancare.com/back-pain-treatment/",
        },
        {
          subTitle: "Knee Pain",
          url: "https://www.nivaancare.com/knee-pain-treatment/",
        },
      ],
    },
    {
      id: 3,
      title: "Quick Links",
      links: [
        {
          subTitle: "Pain Management - Delhi",
          url: "https://www.nivaancare.com/pain-management-clinic-in-delhi/",
        },
      ],
    },
    {
      id: 4,
      titleIcon: locationIcon,
      title: "Our Locations",
      links: [
        {
          subTitle: "South-Delhi",
          url: "https://maps.app.goo.gl/ShnPReST8DKN9gRs5",
        },
        {
          subTitle: "North-Delhi",
          url: "https://maps.app.goo.gl/3Dg5FdmaM87GsS2UA",
        },
      ],
    },
  ];

export const FooterLinks: FooterLinkSection[] = [
    {
      id: 1,
      title: "Quick links",
      links: [
        {
          subTitle: "Privacy Policy",
          url: "https://www.nivaancare.com/privacy-policy/",
        },
        {
          subTitle: "Terms of Use",
          url: "https://www.nivaancare.com/terms-and-conditions/",
        },
      ],
    },
    {
      id: 2,
      title: "Clinic locations",
      // links: [
      //   {
      //     subTitle: "East-Delhi",
      //     url: "https://maps.app.goo.gl/fmNjuaiuNwNQxWbS7",
      //   },
      //   { subTitle: "West-Delhi", url: "https://g.co/kgs/zYFLo96" },
      //   {
      //     subTitle: "North-Delhi",
      //     url: "https://maps.app.goo.gl/3Dg5FdmaM87GsS2UA",
      //   },
      //   {
      //     subTitle: "South-Delhi",
      //     url: "https://maps.app.goo.gl/ShnPReST8DKN9gRs5",
      //   },
      // ],
      links: [
        {
          subTitle: 'Delhi',
          url: ''
        },
        {
          subTitle: 'Faridabad',
          url: ''
        },
        {
          subTitle: 'Gurugram',
          url: ''
        },
        {
          subTitle: 'Ghaziabad',
          url: ''
        },
        {
          subTitle: 'Noida',
          url: ''
        },
        {
          subTitle: 'Lucknow',
          url: ''
        },
        // {
        //   subTitle: 'Meerut',
        //   url: ''
        // },
        {
          subTitle:'Jaipur',
          url:''
        },
        {
          subTitle:'Mumbai',
          url:''
        }
      ]
    },
    {
      id: 3,
      title: "About Us",
      links: [
        { subTitle: "Blogs", url: "https://www.nivaancare.com/blog/" },
        {
          subTitle: "How It Works",
          url: "https://www.nivaancare.com/how-it-works/",
        },
      ],
    },
    {
      id: 4,
      title: "Contact Us",
      links: [
        { subTitle: "care@nivaancare.com", url: "mailto:care@nivaancare.com" },
        { subTitle: "+91-63 6652 5217", url: "tel:+916366525217" },
      ],
    },
  ];
  
  export const FooterLinks4: FooterLinkSection[] = [
    {
      id: 1,
      title: "Quick links",
      links: [
        {
          subTitle: "Privacy Policy",
          url: "https://www.nivaancare.com/privacy-policy/",
        },
        {
          subTitle: "Terms of Use",
          url: "https://www.nivaancare.com/terms-and-conditions/",
        },
      ],
    },
    {
      id: 2,
      title: "Clinic locations",
      links: [
        {
          subTitle: "East-Delhi",
          url: "",
        },
         {
          subTitle: "West-Delhi",
          url: "",
        },
         {
          subTitle: "North-Delhi",
          url: "",
        },
         {
          subTitle: "South-Delhi",
          url: "https://maps.app.goo.gl/ShnPReST8DKN9gRs5",
        }
      ],
    },
    {
      id: 3,
      title: "About Us",
      links: [
        { subTitle: "Blogs", url: "https://www.nivaancare.com/blog/" },
        {
          subTitle: "How It Works",
          url: "https://www.nivaancare.com/how-it-works/",
        },
      ],
    },
    {
      id: 4,
      title: "Contact Us",
      links: [
        { subTitle: "care@nivaancare.com", url: "mailto:care@nivaancare.com" },
        { subTitle: "+91-63 6652 5217", url: "tel:+916366525217" },
      ],
    },
  ];